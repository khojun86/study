        ...
        while True:
            try:
                ...
                if state == 'buy' and nowTime != prevTime:
                    ...
                    print('매수총액 =', boughtedbalance, '보유수량 =', round(boughtedvolume, 2), '평단가 =',
                          round(abp, 2))
                    print('판매가 =', sellprice)
                    if sellprice == 0:
                ...
                if nowTime != prevTime:
                    ...
                    #### 0817 update 매도 주문 exception 발생 시
                    #### 전부매도 된 상태에는 진입하지 않도록####
                    if selluuid != '':
                        curr_order = upbit.get_order(ticker)
                        time.sleep(1)
                        tmpboughtedvolume = float(upbit.get_balances()[1]['locked']) + float(
                            upbit.get_balances()[1]['balance'])
                        time.sleep(1)
                        if tmpboughtedvolume > 0:
                            if len(curr_order) == 0 and data["trade_price"] < sellprice:
                                print('오류남 매수총액 =', boughtedbalance, '보유수량 =', round(boughtedvolume, 2), '평단가 =',
                                      round(abp, 2))
                                sellprice = int(abp * (1 + margin / 100) / tic) * tic
                                print('그래서 매수총액 =', boughtedbalance, '보유수량 =', round(boughtedvolume, 2), '평단가 =',
                                      round(abp, 2))
                                print(time.strftime('%H:%M'), "매도가 ABP+", margin, "% = ", sellprice)
                                sellorder = upbit.sell_limit_order(ticker, sellprice, boughtedvolume)
                                time.sleep(1)
