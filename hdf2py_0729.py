...
# ==========================================
# 2. 데이터 병합 (pd.cut) 및 시각화 엔진 (파일별 옵션 지원)
# ==========================================
def calculate_and_plot_time_series(df_b887, file_window_dict):
    """파일별로 다른 시간 윈도우(window_size)를 적용하여 시계열 그래프를 그립니다."""
    ...
    for file_name, df_group in df_b887.groupby('Source_File'):
        ...
        # 파일별로 지정된 윈도우 사이즈 가져오기 (없으면 100ms 기본값)
        window_size = file_window_dict.get(file_name, '100ms')
        
        if crc_col in df_group.columns:
            df_group = df_group[df_group[crc_col].astype(str).str.contains('Pass', case=False, na=False)]

        df_group['TB Size'] = pd.to_numeric(df_group['TB Size'], errors='coerce').fillna(0)
        df_group['Timestamp'] = pd.to_datetime(df_group['Timestamp'])
        df_time_indexed = df_group.set_index('Timestamp')
        
        df_windowed = df_time_indexed.resample(window_size).agg({'TB Size': 'sum'}).fillna(0)
        window_sec = pd.to_timedelta(window_size).total_seconds()
        df_windowed['Throughput_Mbps'] = (df_windowed['TB Size'] * 8) / window_sec / 1_000_000
        
        ax.plot(df_windowed.index, df_windowed['Throughput_Mbps'], linewidth=1.5, label=f'{file_name} ({window_size})')
        
        df_out = df_windowed.reset_index()
        df_out['Source_File'] = file_name
        df_out['Window_Size'] = window_size
        all_windowed_dfs.append(df_out)
    
    ax.set_title('Time vs PDSCH Throughput', fontsize=14, fontweight='bold')
    ax.set_xlabel('System Time', fontsize=10)
...
def calculate_and_plot_scatter(df_thr, df_ref, file_cols_dict, title, xlabel):
    """파일별로 선택된 컬럼(x_cols)만 가져와서 다중 파일 산점도를 그립니다."""
    fig, ax = plt.subplots(figsize=(12, 5))
    ...
    for file_name in df_ref['Source_File'].unique():
        # [핵심] 해당 파일에서 선택된 옵션(컬럼)이 있는지 확인하고, 하나도 선택 안 했으면 패스
        ...
        if crc_col in df_thr_sub.columns:
            df_thr_sub = df_thr_sub[df_thr_sub[crc_col].astype(str).str.contains('Pass', case=False, na=False)]
            
        df_thr_sub['TB Size'] = pd.to_numeric(df_thr_sub['TB Size'], errors='coerce').fillna(0)
        ...
        grouped = df_thr_sub.groupby('time_bin', observed=False)['TB Size'].sum().reset_index()
        ...
        df_merged = pd.merge(df_ref_sub, grouped, left_on='Timestamp', right_on='ref_time', how='left')
        df_merged['TB Size'] = df_merged['TB Size'].fillna(0)
        df_merged['Throughput_Mbps'] = (df_merged['TB Size'] * 8) / df_merged['duration'] / 1_000_000
        df_merged['Source_File'] = file_name
        all_merged_dfs.append(df_merged)
        
        has_valid_data = False
        # 파일별로 지정된 컬럼만 순회하며 산점도를 그림
        for col in x_cols:
            if col in df_merged.columns:
                valid_data = df_merged.dropna(subset=[col])
                if not valid_data.empty:
                    has_valid_data = True
                    ax.scatter(valid_data[col], valid_data['Throughput_Mbps'], 
                               label=f'{file_name} - {col}', alpha=0.7, edgecolors='k', color=colors[color_idx % len(colors)])
                    color_idx += 1
    ...
    if all_merged_dfs:
        ax.legend(loc='upper right', fontsize=8, bbox_to_anchor=(1.05, 1))
    plt.tight_layout()
