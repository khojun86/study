    ...
    async with websockets.connect(url) as websocket:
        ...
        while True:
            try:
                ...
                #### 30tick close 의 ma 10 가 ma 50 을 돌파할 때 매수####  ## 0817 update
                if state == 'ready':
                    ...
                    if tick30_ma50 < tick30_ma10 and np.isnan(tick30_ma50) == False:
                        print(time.strftime('%H:%M'),'매수시간')
                        buy=upbit.buy_market_order(ticker, buyprice)
                        state = 'buy'
                        buyTime = time.strftime('%H:%M');print(buyTime,state)
                ...
                if state == 'buy' and nowTime != prevTime:
                    ...
                    boughtedvolume = float(upbit.get_balances()[1]['locked']) + float(upbit.get_balances()[1]['balance'])
...
# pprint.pprint(upbit.get_balance(ticker))
